...
import UIGender from '../../UI/UIGender';
  ...
  handle(data: DataView): void {
    ...
    switch (reasonCode) {
      case 0: {
        ...
        console.log('accountId:', accountId, 'female:', female, 'admin:', admin);
        UILogin.hideLoading();
        if (female === 10) {
          // gender not set — show selection
          UIGender.initialize(UILogin.gameCanvas);
          break;
        }
