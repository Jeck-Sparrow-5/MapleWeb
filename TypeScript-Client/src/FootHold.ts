...
class Foothold {
  ...
  load(): void {
    ...
    this.force = wzNode.nGet("force")?.nGet("nValue", 0) ?? 0;
    this.forbid = wzNode.nGet("forbidFallDown")?.nGet("nValue", 0) ?? 0;
    this.cantThrough = wzNode.nGet("cantThrough")?.nGet("nValue", 0) ?? 0;
